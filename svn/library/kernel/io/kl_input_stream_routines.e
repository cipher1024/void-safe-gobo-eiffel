...
feature -- Access
	...
	name (a_stream: like INPUT_STREAM_TYPE): STRING is
		...
		do
			s := a_stream.name
				--| FIXME:jfiat: a_stream.name can be Void ...
			check a_stream_has_name: s /= Void end
			Result := s
		ensure
...
feature -- Input operations
	...
	read_string (a_stream: like INPUT_STREAM_TYPE; nb_char: INTEGER): STRING is
		...
		do
			...
			s := a_stream.last_string
			check s /= Void end -- implied by `read_stream' postcondition
			Result := s
		ensure
